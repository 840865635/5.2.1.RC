...
------------------------------------------------------------*/
function changeAct(tableId,isSubSystem,imatrixUrl){
	newId=getLastNewId(tableId);
	var ids = jQuery('#'+tableId).jqGrid('getDataIDs');
	for(var i=0;i < ids.length;i++){
		var cl = ids[i];
		var ae = "<a href='#pos' class='small-button-bg' onclick=\"myAddRow(newId,'"+cl+"','"+tableId+"','"+isSubSystem+"','"+imatrixUrl+"');\"><span class='ui-icon ui-icon-plusthick'></span></a>";
		var de ="";
		if(deleteUrl!=''){
			de = "<a href='#pos' class='small-button-bg' onclick=\"deleteFormTableData('"+tableId+"','"+cl+"','"+deleteUrl+"','"+isSubSystem+"','"+imatrixUrl+"');\" ><span class='ui-icon  ui-icon-minusthick'></span></a>"; 
		}else{
...
function myAddRow(newId,cl,tableId,isSubSystem,imatrixUrl){
	...
	if(!hasEdit){
		...
		changeAct(tableId,isSubSystem,imatrixUrl);
...
function focusOnClickedCell(rowId,e){
	...
	//$('input,select',e.target).focus();//注释这条语句的原因是因为在IE9下无法获取属性“target”的值,因此在IE9下会报脚本错误
}
/**
 * 删除
 * @return
...
function formGridTreeCallBack(rowid,hiddenField,treeType,isMutiply){
	if(hiddenField!='' && ( treeType=='COMPANY' || treeType=='MAN_DEPARTMENT_GROUP_TREE' || treeType=='MAN_DEPARTMENT_TREE' || treeType=='MAN_GROUP_TREE')){
		 if(isMutiply){
			$("#"+rowid+"_"+hiddenField).attr("value",jstree.getLoginNames());
		 }else{
			$("#"+rowid+"_"+hiddenField).attr("value",jstree.getLoginName());
		 }
	}else{
		if(isMutiply){
			$("#"+rowid+"_"+hiddenField).attr("value",jstree.getIds());
		}else{
			$("#"+rowid+"_"+hiddenField).attr("value",jstree.getId());
		}
	}
}
//表单打印隐藏控件
