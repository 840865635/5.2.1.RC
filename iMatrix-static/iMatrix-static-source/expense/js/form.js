...
//部门人员控件回调方法
function deptSelectCallback(hiddenInputId,showInputId,treeType,multiple){
	...
	}else if(treeType=='COMPANY'||treeType=='MAN_DEPARTMENT_GROUP_TREE'||treeType=='MAN_DEPARTMENT_TREE'||treeType=='MAN_GROUP_TREE'){
		if(multiple=='false'){
	...
	}else if(treeType=='DEPARTMENT_TREE'||treeType=='GROUP_TREE'){
		if(multiple=='false'){
			$("#"+hiddenInputId).attr("value",jstree.getName());
		}else{
			$("#"+hiddenInputId).attr("value",jstree.getNames());
		}
	}
}
...
//显示签章时,处理签章控件的隐藏 显示
function initSignatureControl(signatureVisible){
	if(signatureVisible){
	 var  signatures = __signatureFields.split(",");//签章字段集合，以逗号隔开的
	 for(var j=0;j<signatures.length;j++){
		var signs = $("input[signaturevisible=true]");//获得签章控件集合
