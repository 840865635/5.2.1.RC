					...
					$.ajax({
						   ...
						   success: function(data, textStatus){
							   if( data == "yes" ){
									...
									setPageState();
									ajaxSubmit("ajax_from",webRoot+"/organization/user!clearUser.action?ids="+uIds+"&departmType="+$("#oType").val(),"acs_list");
								}
						},error : function(XMLHttpRequest, textStatus) {
								alert(textStatus);
						}
			...
			<div class="opt-btn">
				...
				<security:authorize ifAnyGranted="acs_clear_user">
					<s:if test='#versionType=="online"'>
						<button  class='btn' onclick="demonstrateOperate();"><span><span>删除</span></span></button>
					</s:if><s:else>
						<button  class='btn' onclick="clearUser();"><span><span>删除</span></span></button>
					</s:else>
		        </security:authorize>
				<security:authorize ifAnyGranted="falseDelete">
					<s:if test='#versionType=="online"'>
						<button id="clear_department_sign" style="display: none;" class='btn' onclick="demonstrateOperate();"><span><span>移除部门</span></span></button>
					</s:if><s:else>
						<button id="clear_department_sign" style="display: none;" class='btn' onclick="opt_delete('${acsCtx}/organization/user!falseDelete.action','NEW');"><span><span>移除部门</span></span></button>
					</s:else>
				</security:authorize>
				<script type="text/javascript">
