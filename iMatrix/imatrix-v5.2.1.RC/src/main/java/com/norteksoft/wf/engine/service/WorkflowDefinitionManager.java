...
public class WorkflowDefinitionManager {
    ...
    /**
     * 创建流程定义（flex用）,用于兼容老版本流程图
     */
    @Transactional(readOnly=false)
    public Long createWfDefinitionById(Long companyId, String xmlFile,Long typeId,Long systemId){
    	ThreadParameters params=new ThreadParameters(companyId);
    	ParameterUtils.setParameters(params);
    	return saveWorkflowDefinition(null, companyId, xmlFile,typeId,systemId);
    }
    ...
    /**
     * 修改流程定义（flex用）,用于兼容老版本流程图
     * @param id
     * @param xmlFile
     */
    @Transactional(readOnly=false)
    public Long updateWfDefinitionById(Long id, Long companyId, String xmlFile,Long typeId,Long systemId){
    	ThreadParameters params=new ThreadParameters(companyId);
    	ParameterUtils.setParameters(params);
    	return saveWorkflowDefinition(id, companyId, xmlFile,typeId,systemId);
    }
...
//					MemCachedUtils.add("document="+wfd.getProcessId(), Dom4jUtils.getDocument(file.getDocument()));
	 ...
	 * 流程监控
	 */
	public void monitorHistory(Page<Object> page,WorkflowDefinition definition) {
		searchMonitorHistory(page,definition,null);
	}
	 ...
	 * 历史流程监控
	 */
	public void monitorDefinitionHistory(Page<Object> page,Long typeId,String defCode) {
		searchManagerMonitorHistory(page,typeId,defCode,null);
	}
	...
	public void searchManagerMonitorHistory(Page<Object> page,
		...
		if(isSuperWf){//是否是流程定义管理员
			hql.append(" from ").append(" HistoryWorkflowInstance wi , WorkflowDefinition wfd  ").append("where wi.processState<>? and wi.workflowDefinitionId=wfd.id and wfd.adminLoginName=? ");
			objs.add(getLoginName());
		}else{
			hql.append(" from ").append(" HistoryWorkflowInstance wi , WorkflowDefinition wfd  ").append("where wi.processState<>? and wi.workflowDefinitionId=wfd.id ");
		}
...
//		}
	 ...
	 * 流程监控(查询)
	 */
	public void searchMonitorHistory(Page<Object> page,
			WorkflowDefinition definition,
			List<ListColumn> fields){
		FormView form = formViewManager.getCurrentFormViewByCodeAndVersion(definition.getFormCode(), definition.getFromVersion());
		monitorDefaultWorkflowDefinitionHistory(page,definition,form,fields);
	}
	
	private void monitorDefaultWorkflowDefinitionHistory(Page<Object> page,
			WorkflowDefinition definition, FormView form, List<ListColumn> searchField) {
		StringBuilder sql = new StringBuilder(
				);
		List<Object> objs = new ArrayList<Object>();
		sql.append(" from ").append(" HistoryWorkflowInstance wi ")
	...
	public List<WorkflowDefinition> getEnableWorkflowDefinitionsByFormCodeAndVersion(String code, Integer version,Long systemId) {
		return workflowDefinitionDao.getEnableWorkflowDefinitionsByFormCodeAndVersion(code, version,systemId);
	}
	...
	public WorkflowDefinition getWfDefinitionByWfdId(Long wfdId){
		return workflowDefinitionDao.get(wfdId);
	}
	...
	/**
     * 根据表单获得相关的状态为启用或草稿的流程定义
     * @param formView
     * @return
     */
    public List<WorkflowDefinition> getWfDefinitionsByForm(FormView formView) {
    	return workflowDefinitionDao.getWfDefinitionsByForm(formView);
    }
