...
<html>
   ...
   <script  type="text/javascript" src="${imatrixCtx}/widgets/calendar/WdatePicker.js"></script>
	...
	function deleteUserLogin(){
		...
		$.post(url, {loginLogIds:uIds}, function(data) {
        	$("#_loading_div_id").hide();
        	showMessage("message", "<font color=\"green\">"+data+"</font>");
			jQuery("#login_log_table").jqGrid().trigger("reloadGrid");
		});
	}
    ...
    function deleteAllUserLogin(){
		...
		$("#_loading_div_id").show();
   		var url="${acsCtx}/log/log!deleteUserLoginLog.action";
   		$.post(url, {deleteAll:"yes"}, function(data) {
        	$("#_loading_div_id").hide();
        	showMessage("message", "<font color=\"green\">"+data+"</font>");
			jQuery("#login_log_table").jqGrid().trigger("reloadGrid");
		});
	...
	<div class="opt-body">
		<div class="opt-btn">
			...
			<s:if test='#versionType=="online"'>
				<security:authorize ifAnyGranted="delete_loginLog"><button  class='btn' onclick="demonstrateOperate();"><span><span>删除</span></span></button></security:authorize>
				<security:authorize ifAnyGranted="delete_loginLog"><button  class='btn' onclick="demonstrateOperate();"><span><span>删除所有</span></span></button></security:authorize>
			</s:if><s:else>
