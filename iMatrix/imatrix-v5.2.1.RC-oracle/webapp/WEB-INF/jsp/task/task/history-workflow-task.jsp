...
<head>
	...
	function taskList(){
		...
		ajaxSubmit('searchForm', encodeURI('${taskCtx}/task/history-workflow-task.htm?typeName='+typeName+'&taskType='+taskType), 'pageTablelist' );
	}
	function changeTaskType(taskType){
		$("#typeName").attr("value","");
		$("#taskType").attr("value",taskType);
		ajaxSubmit('pageForm', '${taskCtx}/task/history-workflow-task.htm', 'pageTablelist_all',changeTaskTypeCallback );
	}
...
<body onclick="$('#sysTableDiv').hide();$('#styleList').hide();">
	<script type="text/javascript">
  		...
  		var thirdMenu = "historyTask-thirdMenu1";
  	</script>
		   ...
		   <aa:zone name="pageTablelist_all">
			...
			<aa:zone name="pageTablelist">
				...
				<form id="searchSubmit" name="searchSubmit" action="" method="post">
					<view:jqGrid url="${taskCtx}/task/history-workflow-task.htm?taskCategory=complete&typeName=${typeName }&taskType=${taskType }" code="HISTORY_WORKFLOW_TASK" gridId="taskCompletedTableId" pageName="page" submitForm="searchForm"></view:jqGrid>
				</form>
			</aa:zone>
			</div>
			</aa:zone>
