function viewDict(id){
	...
	ajaxSubmit("defaultForm",  webRoot+"/engine/data-dictionary!input.htm", "dict_zone", inputCallback);
	$("#dict_id").attr("value", "");
}
...
//移除办理人
function clearAway(){
	if($('#slcMan').children().length==0){
		alert("没有办理人员");return;
	}else{
		var url=webRoot+'/engine/data-dictionary-clearAway.htm';
		if($("input[name=userNames]").length==1 && $("input[name=userNames]").val()=='all_users'){
			url=url+"?sign=all_users";
		}
		init_colorbox(url,"移除办理人");
	}
}
function idSlct(id){
	$("#dict_id").attr("value", id);
