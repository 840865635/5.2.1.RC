...
//		renderText(bl.toString()+taskWebserviceImpl.personalTasks(loginName, companyId, rows));
	...
	public String assignTo() throws Exception{
		...
		//指派任务的业务补偿
		workflowTaskManager.assignTransactorSet(transactor, id);
		ApiFactory.getBussinessLogService().log("待办事宜", 
				"指派任务", 
				ContextUtils.getSystemId("task"));
		log.debug("*** receive 方法结束");
...
//		String url="http://192.168.1.98:8088/imatrix/"+task.getUrl()+task.getId();
	...
	public String messageTask() throws Exception{
		...
		if(id==null){
			response.sendRedirect(SystemUrls.getSystemPageUrl("imatrix")+"/portal/my-message-error.action?errorInfo="+URLEncoder.encode("该任务不存在","UTF-8") );
		}else{
			com.norteksoft.product.api.entity.WorkflowTask mytask=ApiFactory.getTaskService().getTask(id);
			if(mytask==null){
				response.sendRedirect(SystemUrls.getSystemPageUrl("imatrix")+"/portal/my-message-error.action?errorInfo="+URLEncoder.encode("该任务不存在","UTF-8") );
			}else{
				if(mytask.getPaused()){
					response.sendRedirect(SystemUrls.getSystemPageUrl("imatrix")+"/portal/my-message-error.action?errorInfo="+URLEncoder.encode("该任务对应的实例已暂停","UTF-8") );
				}else{
					if(mytask.getActive().equals(2)){
						response.sendRedirect(SystemUrls.getSystemPageUrl("imatrix")+"/portal/my-message-error.action?errorInfo="+URLEncoder.encode("该任务已完成","UTF-8") );
						return null;
					}if(mytask.getActive().equals(3)){
						response.sendRedirect(SystemUrls.getSystemPageUrl("imatrix")+"/portal/my-message-error.action?errorInfo="+URLEncoder.encode("该任务已取消","UTF-8") );
						return null;
					}else if(mytask.getActive().equals(5)){
						response.sendRedirect(SystemUrls.getSystemPageUrl("imatrix")+"/portal/my-message-error.action?errorInfo="+URLEncoder.encode("该任务已被指派","UTF-8") );
						return null;
					}else if(mytask.getActive().equals(7)){
						response.sendRedirect(SystemUrls.getSystemPageUrl("imatrix")+"/portal/my-message-error.action?errorInfo="+URLEncoder.encode("该任务已被他人领取","UTF-8") );
						return null;
					}else{
