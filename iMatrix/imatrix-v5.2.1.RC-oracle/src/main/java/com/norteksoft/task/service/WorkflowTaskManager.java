...
public class WorkflowTaskManager implements WorkflowTaskService{
	...
	public Integer getDelegateTasksNumByActive(Long companyId, String loginName, Boolean isCompleted){
		if(isCompleted){
			Integer currentNum =  workflowTaskDao.getDelegateTasksNum(companyId, loginName, isCompleted);
			Integer histNum = historyWorkflowTaskDao.getDelegateTasksNum(companyId, loginName);
	...
	public Integer getTrusteeTasksNum(Long companyId, String loginName, Boolean isCompleted){
		if(isCompleted){
			Integer currentNum = workflowTaskDao.getTrusteeTasksNum(companyId, loginName, isCompleted);
			Integer histNum = historyWorkflowTaskDao.getTrusteeTasksNum(companyId, loginName);
			if(currentNum == null)currentNum = 0;
			if(histNum == null)histNum = 0;
			return currentNum+histNum;
		}else{
	...
	private void generateAssignHistory(WorkflowTask task, String transactor,WorkflowTask delegateTask,String trustee) {
		...
		if(StringUtils.isNotEmpty(trustee)){//生成指派和委托的流转历史
			...
			inforSeven = acsUtils.getUserByLoginName(transactor).getName();
			inforEight = acsUtils.getUserByLoginName(transactor).getLoginName();
			...
			inforThree = acsUtils.getUserByLoginName(transactor).getName();
			inforFour = acsUtils.getUserByLoginName(transactor).getLoginName();
	...
	public List<String> getTransactorsByTask(String name,String transactor,String processId){
		return workflowTaskDao.getTransactorsByTask(name,transactor,processId);
	}
}
