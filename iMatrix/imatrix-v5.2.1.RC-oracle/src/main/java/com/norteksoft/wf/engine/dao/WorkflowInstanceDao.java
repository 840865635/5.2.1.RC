...
public class WorkflowInstanceDao extends HibernateDao<WorkflowInstance, Long>{
	...
	public List<WorkflowInstance>  getInstancesByFormId(Long formId){
		String hql = "from WorkflowInstance wi where  wi.formId = ?";
	...
	public List<Object> getActivetySubProcess(String processInstanceId) {
		...
		newHql.append(" and w.parentProcessId=? and (p.active=0 or p.active=1 or p.active=4 or p.active=6) and p.processingMode!=?");

		return this.find(newHql.toString(), processInstanceId,TaskProcessingMode.TYPE_READ);
	}
	public List<WorkflowInstance> getInstanceByCode(String processCode){
		return find("from WorkflowInstance wfi where wfi.processCode=?", processCode);
	}
	 ...
	 * @return
	 */
	public List<WorkflowInstance> getEndAndCancelProcess(){
		return find("from WorkflowInstance wi where wi.companyId = ? and (wi.processState=? or wi.processState=?) order by wi.submitTime ", 
				ContextUtils.getCompanyId(),ProcessState.END,ProcessState.MANUAL_END);
	}
}
