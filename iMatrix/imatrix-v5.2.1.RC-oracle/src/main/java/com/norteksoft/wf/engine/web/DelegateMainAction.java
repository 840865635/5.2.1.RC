...
public class DelegateMainAction extends CrudActionSupport<TrustRecord>{
	...
	private Boolean inHistory=false;//是否是历史实例任务
	@Autowired
	...
	public void prepareView() throws Exception {
		if(id==null){
			...
			workflowVersion = workflowDefinitionManager.getWorkflowDefinitionByProcessId(delegateMain.getProcessId()).getVersion();
		}
	 ...
	 * @throws Exception
	 */
	public String myHistoryDelegate()throws Exception{
		if(historyTasks.getPageSize()>1){
			historyTasks = taskService.getHistoryDelegateTasksByActive(
					ContextUtils.getCompanyId(), historyTasks, ContextUtils.getLoginName());
	...
	public String superviseAsTrustee()throws Exception{
		if(inHistory){
			if(historyTasks.getPageSize()>1){
				historyTasks = historyWorkflowTaskManager.getTaskAsTrustee(
						ContextUtils.getCompanyId(), historyTasks, ContextUtils.getLoginName(), isDone);
				ApiFactory.getBussinessLogService().log("委托", 
						"任务受托监控列表", 
						ContextUtils.getSystemId("wf"));
				this.renderText(PageUtils.pageToJson(historyTasks));
	...
	public void setHistoryTasks(Page<HistoryWorkflowTask> historyTasks) {
