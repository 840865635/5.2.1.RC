...
public class TreeAction extends CrudActionSupport {
	...
	public String wfTypes() throws Exception{
		...
		if( "INITIALIZED_PROCESS".equals(currentId)){
			...
			tree.append(JsTreeUtils.generateJsTreeNodeDefault("WFT_monitor_0", "open", 
					"当前实例",getSecondNodesInMonitorTree(wfTypes))).append(",");
			tree.append(JsTreeUtils.generateJsTreeNodeDefault("WFT_history_monitor_0", "", 
					"归档实例",getSecondNodesInMonitorHistoryTree(wfTypes))).append(",");
	 ...
	 public String getSecondNodesInMonitorTree(List<WorkflowType> wfTypes){
		StringBuilder tree = new StringBuilder();
 ...
 public String getSecondNodesInMonitorHistoryTree(List<WorkflowType> wfTypes){
		StringBuilder tree = new StringBuilder();
			tree.append(JsTreeUtils.generateJsTreeNodeDefault("WFT_history_monitor_0", null, 
			"所有流程")).append(",");
			boolean isSuperWf=workflowDefinitionManager.isSuperWf();
			for(WorkflowType wft : wfTypes){
				tree.append(JsTreeUtils.generateJsTreeNodeDefault("WFT_history_monitor_"+wft.getId(), null, wft.getName(),monitorHistoryTree(wft,isSuperWf))).append(",");
			}
		JsTreeUtils.removeLastComma(tree);
		return tree.toString();
	}
	...
	private String monitorHistoryTree(WorkflowType type,boolean isSuperWf){
		StringBuilder tree = new StringBuilder();
		List<String> definitionCodes=workflowDefinitionManager.getWfDefinitionCodesByType(getCompanyId(), type.getId());
		for(String def : definitionCodes){
			WorkflowDefinition wf=workflowDefinitionManager.getWorkflowDefinitionByCodeAndVersion(def, 1,ContextUtils.getCompanyId(),isSuperWf);
			if(wf!=null){
				tree.append(JsTreeUtils.generateJsTreeNodeDefault("WFT_history_monitor_"+type.getId()+"_"+def, null, wf.getName())).append(",");
			}
		}
		JsTreeUtils.removeLastComma(tree);
		return tree.toString();
