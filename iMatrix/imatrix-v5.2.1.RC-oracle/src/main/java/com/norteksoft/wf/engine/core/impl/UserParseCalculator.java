					...
					if(StringUtils.isNotEmpty(fieldValue)){
						if("ALLWORKGROUP".equals(fieldValue)){//所有工作组中的人员,只有在条件为【人员  等于 “ 标准字段的值”】时才有用
							...
							for(Workgroup wg:workgroups){
								boolean result = UserUtil.userInWorkGroup(ContextUtils.getCompanyId(), loginName, wg.getName());
								if(result)return result;
							}
						}else{
							boolean result = UserUtil.userInWorkGroup(ContextUtils.getCompanyId(), loginName, fieldValue);
							if(result)return result;
						}
					}
				}
				...
				for(String fieldValue:departmentName.split(",")){//字段中选择了多个部门时
					...
					if(StringUtils.isNotEmpty(fieldValue)){
						if("所有人员".equals(fieldValue)||"ALLCOMPANYID".equals(fieldValue)){//所有人员(不包含无部门人员),只有在条件为【人员  等于 “ 标准字段的值”】时才有用
							...
							boolean result = UserUtil.userInDepartment(ContextUtils.getCompanyId(), loginName, fieldValue);
							if(result)return result;
						}
					}
				}
	...
	private  boolean parseUser(String atomicExpress,String loginName){
		if(StringUtils.contains(atomicExpress, TextOperator.NET.getCode())) {
			...
			if(StringUtils.isNotEmpty(userLoginName)){
				for(String fieldValue:userLoginName.split(",")){
					fieldValue = fieldValue.trim();
					if(StringUtils.isNotEmpty(fieldValue)){
						if("ALLCOMPANYID".equals(fieldValue)){//所有人员(不包含无部门人员),只有在条件为【人员  等于 “ 标准字段的值”】时才有用
							...
							List<String> loginNames = ApiFactory.getAcsService().getLoginNamesByWorkgroup(ContextUtils.getCompanyId());
							for(String name:loginNames){
								//zhangsan,lisi 当前为lisi，应返回false，说明当前用户loginName包含在结果集中
								boolean result = loginName.equals(name);
								if(result) return false;
							}
						}else{
							//zhangsan,lisi 当前为lisi，应返回false，说明当前用户loginName包含在结果集中
							boolean result = loginName.equals(fieldValue);
							if(result) return false;
						}
					}
				}
			...
			for(String fieldValue:userLoginName.split(",")){
				...
				if(StringUtils.isNotEmpty(fieldValue)){
					if("ALLCOMPANYID".equals(fieldValue)){//所有人员(不包含无部门人员),只有在条件为【人员  等于 “ 标准字段的值”】时才有用
						...
						return loginName.equals(fieldValue);
					}
				}
			}
		}
				...
				for(String fieldValue:workGroupName.split(",")){
					...
					if(StringUtils.isNotEmpty(fieldValue)){
						if("ALLWORKGROUP".equals(fieldValue)){//所有工作组中的人员,只有在条件为【人员  等于 “ 标准字段的值”】时才有用
							...
							workGroupSet.add(UserUtil.getWorkGroupByName(fieldValue));
						}
					}
				}
			}
				...
				for(String fieldValue:departmentName.split(",")){//字段中选择了多个部门时
					...
					if(StringUtils.isNotEmpty(fieldValue)){
						if("所有人员".equals(fieldValue)||"ALLCOMPANYID".equals(fieldValue)){//所有人员(不包含无部门人员),只有在条件为【人员  等于 “ 标准字段的值”】时才有用
							...
							Department department = UserUtil.getDepartmentByName(fieldValue);
		...
		if(StringUtils.contains(condition, TextOperator.NET.getCode())) {
			...
			if(StringUtils.isNotEmpty(roleName)){
				for(String fieldValue:roleName.split(",")){//当是字段中的角色时，会有逗号
					fieldValue = fieldValue.trim();
					if(StringUtils.isNotEmpty(fieldValue)){
						roleNames.add(fieldValue);
					}
				}
				List<String> allRoles = UserUtil.getRoleCodesBySystem(systemId);
				//去掉当前的角色名称集合，剩下符合条件的角色集合
				allRoles.removeAll(roleNames);
				for(String role:allRoles){//循环角色，获得角色对应的人员集合
					userSet.addAll(UserUtil.getUsersByRoleName(systemId,role));
				}
			}
		}else if(StringUtils.contains(condition, TextOperator.ET.getCode())){
			String roleName = getRoleName(StringUtils.substringBetween(condition, SINGLE_QUOTATION_MARK, SINGLE_QUOTATION_MARK));
			if(StringUtils.isNotEmpty(roleName)){
				for(String fieldValue:roleName.split(",")){//当是字段中的角色时，会有逗号
					fieldValue = fieldValue.trim();
					if(StringUtils.isNotEmpty(fieldValue)){
						userSet.addAll(UserUtil.getUsersByRoleName(fieldValue,systemId,companyId));
					}
				}
			}
	...
	private Set<String> parseUser(String condition){
		...
		if(StringUtils.contains(condition, TextOperator.NET.getCode())) {
			...
			if(StringUtils.isNotEmpty(loginName)){
				for(String fieldValue:loginName.split(",")){
					fieldValue = fieldValue.trim();
					if(StringUtils.isNotEmpty(fieldValue)){
						if("ALLCOMPANYID".equals(fieldValue)){//所有人员(不包含无部门人员),只有在条件为【人员  等于 “ 标准字段的值”】时才有用
							return userSet;
							...
							List<String> loginNames = ApiFactory.getAcsService().getLoginNamesByWorkgroup(ContextUtils.getCompanyId());
							List<String> allLoginNames = ApiFactory.getAcsService().getLoginNamesByCompany(ContextUtils.getCompanyId());
							//移除工作组中的所有人
							allLoginNames.removeAll(loginNames);
							userSet.addAll(allLoginNames);
							return userSet;
						}else{
							eqNames.add(fieldValue);
						}
					}
				}
				List<String> allLoginNames = ApiFactory.getAcsService().getLoginNamesByCompany(ContextUtils.getCompanyId());
				//移除选择的人员
				allLoginNames.removeAll(eqNames);
				userSet.addAll(allLoginNames);
			}
		}else if(StringUtils.contains(condition, TextOperator.ET.getCode())){
			String loginName = getUserLoginName(StringUtils.substringBetween(condition, SINGLE_QUOTATION_MARK, SINGLE_QUOTATION_MARK));
			if(StringUtils.isNotEmpty(loginName)){
				for(String fieldValue:loginName.split(",")){
					...
					if(StringUtils.isNotEmpty(fieldValue)){
						if("ALLCOMPANYID".equals(fieldValue)){//所有人员(不包含无部门人员),只有在条件为【人员  等于 “ 标准字段的值”】时才有用
							List<String> loginNames = ApiFactory.getAcsService().getLoginNamesByCompany(ContextUtils.getCompanyId());
							userSet.addAll(loginNames);
							return userSet;
						}else if("ALLWORKGROUP".equals(fieldValue)){//所有工作组中的人员,只有在条件为【人员  等于 “ 标准字段的值”】时才有用
							List<String> loginNames = ApiFactory.getAcsService().getLoginNamesByWorkgroup(ContextUtils.getCompanyId());
							userSet.addAll(loginNames);
