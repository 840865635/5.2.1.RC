...
public class InstanceHistoryManager {
	...
	@Autowired
	public void setHistoryInstanceHistoryDao(
			HistoryInstanceHistoryDao historyInstanceHistoryDao) {
		this.historyInstanceHistoryDao = historyInstanceHistoryDao;
	}
	/**
	 * 保存流转历史
	 * @param ih
	...
	public String[][] getHistoryByInstanceId(Long companyId, String instanceId){
		boolean isInstanceComplete = workflowInstanceManager.isInstanceInHistory(instanceId,companyId);
		if(isInstanceComplete){//实例在历史实例表中
			return getHistoryHistoryByInstanceId(companyId,instanceId);
		}else{//实例在当前实例表中
			return getCurrentHistoryByInstanceId(companyId,instanceId);
		}
	}
	
	/**
	 * 根据当前流程实例ID查询历史记录（flex用）
	 * @param companyId
	 * @param instanceId
	 * @return 	String[][]: [环节名称，办理人，操作，办理意见];自动环节没有办理人
	 */
	public String[][] getCurrentHistoryByInstanceId(Long companyId, String instanceId){
	...
	/**
	 * 根据历史流程实例ID查询历史记录（flex用）
	 * @param companyId
	 * @param instanceId
	 * @return 	String[][]: [环节名称，办理人，操作，办理意见];自动环节没有办理人
	 */
	public String[][] getHistoryHistoryByInstanceId(Long companyId, String instanceId){
		List<Object[]> specialHistorieAndOpinions = historyInstanceHistoryDao.getHistoryBySpecial(companyId, instanceId, true);
		List<Object[]> historieAndOpinions = historyInstanceHistoryDao.getHistoryBySpecial(companyId, instanceId, false);

		List<HistoryInstanceHistory> specialHistories = getHistoryHistoryFromHistoryAndOpinion(specialHistorieAndOpinions);
		List<HistoryInstanceHistory> histories = getHistoryHistoryFromHistoryAndOpinion(historieAndOpinions);
	...
	public List<HistoryInstanceHistory> getHistoryHistorysByWorkflowId(Long companyId, String workflowId){
		List<Object[]> histories=historyInstanceHistoryDao.getHistoryByWorkflowId(companyId, workflowId);
		return getHistoryHistoryFromHistoryAndOpinion(histories);
	}
	...
	private List<HistoryInstanceHistory> getHistoryHistoryFromHistoryAndOpinion(List<Object[]> histories){
		List<HistoryInstanceHistory> list = new ArrayList<HistoryInstanceHistory>();
		HistoryInstanceHistory ih = null;
		HistoryOpinion opinion = null;
		Long lastTaskId = null;
		for(int i = 0; i < histories.size(); i++){
			opinion = (HistoryOpinion) histories.get(i)[1];
			//处理同一条流转历史多条意见
			if(opinion != null && lastTaskId != null && lastTaskId.equals(opinion.getTaskId())){
				if(ih.getTransactorOpinion() != null){
					ih.setTransactorOpinion(ih.getTransactorOpinion()+"；"+opinion.getOpinion());
				...
				ih = (HistoryInstanceHistory) histories.get(i)[0];
				if(opinion != null) ih.setTransactorOpinion(opinion.getOpinion());
				lastTaskId = ih.getTaskId();
				list.add(ih);
			}
