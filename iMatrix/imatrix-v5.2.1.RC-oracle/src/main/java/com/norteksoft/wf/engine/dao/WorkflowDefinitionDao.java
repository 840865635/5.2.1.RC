...
public class WorkflowDefinitionDao extends HibernateDao<WorkflowDefinition, Long>{
	...
	public List<WorkflowDefinition> getEnableWorkflowDefinitionsByFormCodeAndVersion(String formCode, Integer version,Long systemId) {
		return find("from WorkflowDefinition wfd where wfd.companyId = ? and wfd.systemId = ? and wfd.enable = ? and wfd.formCode = ? and wfd.fromVersion = ? order by wfd.version desc",
				ContextUtils.getCompanyId(), systemId,DataState.ENABLE, formCode, version);
	}
    ...
    public List<WorkflowDefinition> getWfDefinitionsByForm(FormView formView) {
		...
		hql.append("from WorkflowDefinition wfd where wfd.companyId = ? ");
		hql.append(" and (wfd.enable=? or wfd.enable=?) ");
		hql.append(" and wfd.formCode=? ");
		hql.append("and wfd.fromVersion=? ");
		hql.append(" order by wfd.code,wfd.version desc");
		return find(hql.toString(),ContextUtils.getCompanyId(),DataState.ENABLE,DataState.DRAFT,formView.getCode(),formView.getVersion());
	}
	...
	/**
	 * 根据formCode和formVersion删除流程定义
	 * @param
	 * @param
	 * @return
	 */
	public void deleteWorkflowDefinitionByFormCodeAndFormVersion(String formCode, Integer formVersion) {
		this.createQuery("delete  from WorkflowDefinition t where  t.formCode = ? and t.fromVersion = ? and t.companyId = ?", formCode,formVersion,ContextUtils.getCompanyId()).executeUpdate();
	}
}
