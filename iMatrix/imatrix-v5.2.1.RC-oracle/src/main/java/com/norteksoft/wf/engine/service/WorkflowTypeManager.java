...
public class WorkflowTypeManager {
	...
	public WorkflowType getWorkflowType(String typeCode) {
		if(StringUtils.isEmpty(typeCode)) throw new RuntimeException("没有给定查询流程类型的查询条件：流程类型编号");
	 ...
	 * 根据类型编号获得流程类型(flex使用)
	 */
	public WorkflowType getWorkflowTypeByCode(String typeCode,Long companyId) {
		if(StringUtils.isEmpty(typeCode)) throw new RuntimeException("没有给定查询流程类型的查询条件：流程定义编号");
		return workflowTypeDao.getWorkflowType(typeCode,companyId);
	}
}
