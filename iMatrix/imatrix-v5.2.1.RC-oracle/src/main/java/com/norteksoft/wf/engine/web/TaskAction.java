...
public class TaskAction extends CrudActionSupport<WorkflowTask>{
	...
	private String exceptionSelectFlag="false";//用于标识当是批量环节跳转时，且由于异常情况而无法获得办理人时
	...
	public String opinion() throws Exception {
		...
		if(workflowRightsManager.mustOpinionRight(BeanUtil.turnToModelTask(task)) && (opinions==null || opinions.size()<=0) ){
			must=true;
		}else{
			must=false;
		}
	...
	private String preView(String operate){
		...
		formHtml =workflowInstanceManager.setValueForHtml(workflowInstance.getDataId(), form,form.getHtml()); 
		formHtml = builder.toString() + formHtml;
		...
		if(transactNum>1){
			...
			for(WorkflowTask t : taskList){
				if(StringUtils.isEmpty(t.getTrustorName())){
				  tempTree.append(JsTreeUtil1.generateJsTreeNodeNew("user_"+ t.getTransactorName()+"-"+t.getTransactor(), "",t.getTransactorName(),"user")).append(",");
				}else{
				  tempTree.append(JsTreeUtil1.generateJsTreeNodeNew("user_"+ t.getTransactorName()+"-"+t.getTransactor(), "",t.getTransactorName()+"（受托于"+t.getTrustorName()+")","user")).append(",");
				}
			}
	...
	private String delComma(String str){
		if(StringUtils.isNotEmpty(str)){
			...
			if(str.charAt(length-1)==',')str= str.substring(0,str.length() - 1);
		}
		return str;
	}	
