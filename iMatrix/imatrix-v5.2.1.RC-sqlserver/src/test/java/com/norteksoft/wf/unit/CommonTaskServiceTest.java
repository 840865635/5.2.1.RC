...
public class CommonTaskServiceTest extends BaseWorkflowTestCase {
	...
	public void createTask(){
		ApiFactory.getCommonTaskService().createTask("/aa/bb.html","wangjing_task_name","wangjing_task_title","王晶的类型","王晶");
	...
	public void createTaskTwo(){
		ApiFactory.getCommonTaskService().createTask("wangjing_task_name","wangjing_taks_title", "王晶的类型","王晶");
		Task result = taskDao.findUniqueBy("name", "wangjing_task_name");
		Assert.assertNotNull(result);
		Assert.assertEquals("王晶的类型", result.getCategory());
	}
