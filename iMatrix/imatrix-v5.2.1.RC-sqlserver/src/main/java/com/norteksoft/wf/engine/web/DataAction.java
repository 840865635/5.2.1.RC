...
public class DataAction  extends CrudActionSupport<Object> {
	...
	public String save() throws Exception {
		...
		com.norteksoft.product.api.entity.WorkflowTask task = ApiFactory.getTaskService().getTask(Long.valueOf(resultMap.get(WorkflowInstanceManager.TASKID)));
		taskId = task.getId();
		com.norteksoft.product.api.entity.WorkflowInstance workflowInstance = ApiFactory.getInstanceService().getInstance(workflowInstanceId);
		...
		fieldPermission = ApiFactory.getFormService().getFieldPermission(Long.valueOf(resultMap.get(WorkflowInstanceManager.TASKID)));		
		return "inputForm";
	}
	...
	public String submit() throws Exception {
		...
		com.norteksoft.product.api.entity.WorkflowTask task = ApiFactory.getTaskService().getTask(Long.valueOf(resultMap.get(WorkflowInstanceManager.TASKID).toString()));
		com.norteksoft.product.api.entity.WorkflowInstance workflowInstance = ApiFactory.getInstanceService().getInstance(workflowInstanceId);
		StringBuilder builder = new StringBuilder();
		builder.append( "<input type=\"hidden\" name=\"dataId\" value=\"").append(resultMap.get(WorkflowInstanceManager.DATAID)).append("\"/>");
		formHtml = workflowInstanceManager.getHtml(workflowInstance.getProcessDefinitionId(),workflowInstance.getFormId(),workflowInstance.getDataId(), task.getName());
