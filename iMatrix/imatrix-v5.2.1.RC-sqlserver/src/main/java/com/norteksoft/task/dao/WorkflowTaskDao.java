...
public class WorkflowTaskDao extends HibernateDao<WorkflowTask, Long>{
	...
	public List<String> getTransactorsByTask(String name,String transactor,String processId){
		String hql="select distinct t.transactor from WorkflowTask t where t.name=? and t.transactor=? and t.processInstanceId=? and t.active=?  and t.paused=?";
		return this.find(hql, name,transactor,processId,TaskState.WAIT_TRANSACT.getIndex(),false);
	}
	/**
	 * 根据“任务组”查询任务列表
	 * @param companyId
