...
public class ModulePageDao extends HibernateDao<ModulePage, Long> {
	...
	public ModulePage getModulePageByPageId(Long pageId) {
		List<ModulePage> modules = this.find("from ModulePage v where v.enableState=? and v.companyId=? and v.id=? and v.deleted=?",DataState.ENABLE,ContextUtils.getCompanyId(),pageId,false);
		if(modules.size()>0){
	...
	public void getModulePagesByMenuId(Page<ModulePage> page,Long menuId) {
		this.findPage(page,"from ModulePage v where v.menuId=? and v.companyId=? and v.deleted=? order by v.code", menuId,ContextUtils.getCompanyId(),false);
	}
	...
	public ModulePage getDefaultDisplayPageByMenuId(Long menuId){
		List<ModulePage> pages = this.find("from ModulePage v where v.menuId=? and v.companyId=? and v.defaultDisplay=?  and v.enableState=? and v.deleted=?", menuId,ContextUtils.getCompanyId(),true,DataState.ENABLE,false);
	...
	public List<ModulePage> getEnableModulePagesByMenuId(Long menuId) {
		return this.find("from ModulePage v where v.menuId=? and v.enableState=? and v.companyId=? and v.enableState=? and v.deleted=? order by v.id", menuId,DataState.ENABLE,ContextUtils.getCompanyId(),DataState.ENABLE,false);
	}
	
	/**
	 * 根据菜单id获得状态为草稿或启用的页面
	 */
	public List<ModulePage> getDraftOrEnablePagesByMenuId(Long menuId) {
		return this.find("from ModulePage v where v.companyId=? and v.menuId=? and (v.enableState=? or v.enableState=?) and v.deleted=? ", ContextUtils.getCompanyId(),menuId,DataState.DRAFT,DataState.ENABLE,false);
	}
}
