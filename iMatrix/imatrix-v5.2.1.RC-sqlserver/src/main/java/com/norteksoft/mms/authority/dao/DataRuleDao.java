...
public class DataRuleDao extends HibernateDao<DataRule, Long>{
	...
	public void getDataRulesByMenuId(Page<DataRule> page, Long menuId) {
		this.searchPageByHql(page, "from DataRule d where d.companyId=? and d.menuId=? ", ContextUtils.getCompanyId(),menuId);
	}
	/**
	 * 根据规则类型查询数据规则
	 * @param page
	...
	public List<DataRule> getDataRulesByMenuId(Long menuId) {
		return this.find("from DataRule d where d.companyId=? and d.menuId=? ", ContextUtils.getCompanyId(),menuId);
	}
	/**
	 * 根据系统id获得数据规则
	 * @param systemIds
