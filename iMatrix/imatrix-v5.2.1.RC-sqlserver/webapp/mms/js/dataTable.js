...
function changeTableInfo(url,callback){
	...
	if(ids==""){
		...
		if(versionType=="online"){
			var state=jQuery("#dataTables").jqGrid("getCell",ids,"tableState");
			if(state!= "DRAFT"){
				alert("为确保系统的正常演示,不能修改已启用和禁用的数据表");
				return;
			}
		}
	...
	if(ids==''){
		...
		if(versionType=="online"){
			$.each(ids, function(i){
				...
				if(state!= "DRAFT"){
					alert("为确保系统的正常演示,不能删已启用和禁用的数据表");
					canPost = false;
				}
			});
		}
		if(canPost){
			if(confirm("确定删除吗？")){
				...
				setPageState();
				ajaxSubmit("contentFrom", url, "dataTablelist",deleteCallBack);
...
function selectParent(){
	$.colorbox({href:webRoot+"/form/data-table-selectParent.htm?menuId="+$("#men_id").val()+"&tableId="+$("#tabelId").val(),iframe:true, innerWidth:500, innerHeight:400,overlayClose:false,title:"选择父类"});
}
