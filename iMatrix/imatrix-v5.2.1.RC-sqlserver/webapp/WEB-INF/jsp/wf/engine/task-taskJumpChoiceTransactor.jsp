...
<head>
	...
	function okBtn(){
		var item =$("[name=tranName]:checked");
		if(item.length<=0){
			alert("请选择办理人");
		}else{
			...
			<div id="opt-content" class="form-bg">
				...
				<s:if test="canChoiceTransactor.size>0">
					...
					<table class="form-table-border-left">
						...
						<tbody>
							<s:iterator value="canChoiceTransactor.keys" id="loginName">
								<tr> 
								<td>
									<input type="checkbox" name="tranName" value="<s:property value="#loginName"/>" onclick = "_click_one('_all','tranName');"/>
								</td><td><s:property value="canChoiceTransactor.get(#loginName)"/></td><td><s:property value="#loginName"/></td>
								</tr>
							</s:iterator>
						</tbody>
					</table>
