...
<head>
	...
	$(document).ready(function() {
		historyTree("canceled_tree_id");
	});
	function markColor(code){
		$("#identifiersDiv").hide();
		if(validateReadioTaskIds()){
			$('#taskMark').attr('value', code.toUpperCase());
			ajaxSubmit("task_form", "${taskCtx }/task/history-workflow-task-mark.htm", "pageTablelist");
		}
	}
	...
	function taskList(){
		...
		ajaxSubmit('searchForm', encodeURI('${taskCtx}/task/history-workflow-task-canceled.htm?typeName='+typeName+'&taskType='+taskType), 'pageTablelist' );
	}
	function changeTaskType(taskType){
		$("#typeName").attr("value","");
		$("#taskType").attr("value",taskType);
		ajaxSubmit('pageForm', '${taskCtx}/task/history-workflow-task-canceled.htm', 'pageTablelist_all',changeTaskTypeCallback );
	}
...
<body onclick="$('#sysTableDiv').hide();$('#styleList').hide();">
	<script type="text/javascript">
  		var secondMenu = "historyWorkflowTask";
  		var thirdMenu = "historyTask-thirdMenu2";
  	</script>
  	<%@ include file="/menus/header.jsp"%>
	<%@ include file="/menus/task-sec-menu.jsp"%>
	<div class="ui-layout-west">
		<%@ include file="/menus/historyTask-third-menu.jsp"%>
	</div>
		   ...
		   <aa:zone name="pageTablelist_all">
			...
			<aa:zone name="pageTablelist">
				...
				<form id="searchSubmit" name="searchSubmit" action="" method="post">
					<view:jqGrid url="${taskCtx}/task/history-workflow-task-canceled.htm?taskCategory=cancel&typeName=${typeName }&taskType=${taskType }" code="HISTORY_WORKFLOW_TASK_CANCELED" gridId="taskCompletedTableId" pageName="page" submitForm="searchForm"></view:jqGrid>
				</form>
			</aa:zone>
			</div>
			</aa:zone>
