...
function fmtTitle(ts1,cellval,opts,rwdat,_act){
	...
	if(opts.read == 'true'){
		v ="<img src=\""+webRoot+"/images/"+name+".gif\"/><a  href=\"#\" hidefocus=\"true\" onclick=\"_click_fun("+opts.id+");\">" +value + "</a>";
	}else if(opts.read == 'false'){
		v="<img src=\""+webRoot+"/images/"+name+".gif\"/><a  href=\"#\" hidefocus=\"true\" onclick=\"_click_fun("+opts.id+");\" style=\"font-weight:bold;\">" + value + "</a>";
	}
	return v;
}
...
function fmtFinishedTitle(ts1,cellval,opts,rwdat,_act){
	...
	if(opts.trustorName!="&nbsp;"&&typeof(opts.trustorName)!="undefined"&&opts.trustorName!=""){
	 ...
	 var v ="<img src=\""+webRoot+"/images/"+name+".gif\"/><a  href=\"#\" hidefocus=\"true\" onclick=\"_click_fun("+opts.id+");\">" +value + "</a>";
	return v;
}
/*---------------------------------------------------------
函数名称:fmtName
		...
		"ui" : {    
   		...
   		"plugins" : [ "themes", "json_data","ui" ]
			 ...
			 if(!refreshable){//如果不是办理任务后刷新页面
				 ...
				 historySelectNode($(node).parent().attr("id"),treeid);
			 }
			 refreshable=false;
		  });
}
