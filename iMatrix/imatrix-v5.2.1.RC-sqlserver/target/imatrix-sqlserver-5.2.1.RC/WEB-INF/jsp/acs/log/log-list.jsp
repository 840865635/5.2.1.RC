   ...
   //删除系统日志
	function deleteSysLogin(){
		...
		$("#_loading_div_id").show();
 		...
 		$.post(url, {sysId:$("#sysId").val(),syIds:uIds+"="+sysId}, function(data) {
        	$("#_loading_div_id").hide();
        	showMessage("message", "<font color=\"green\">"+data+"</font>");
			jQuery("#log_table").jqGrid().trigger("reloadGrid");
		});
	}
	...
	function deleteAllSysLogin(){
		...
		$("#_loading_div_id").show();
 		var url="${acsCtx}/log/log!deleteSysLoginLog.action";
 		$.post(url, {sysId:$("#sysId").val(),deleteAllSysLog:"yes",syIds:sysId}, function(data) {
        	$("#_loading_div_id").hide();
        	showMessage("message", "<font color=\"green\">"+data+"</font>");
			jQuery("#log_table").jqGrid().trigger("reloadGrid");
		});
...
<div class="opt-body">
	<div class="opt-btn">
		...
		<s:if test='#versionType=="online"'>
			<security:authorize ifAnyGranted="delete_sysLogs"><button  class='btn' onclick="demonstrateOperate();"><span><span>删除</span></span></button></security:authorize>
			<security:authorize ifAnyGranted="delete_sysLogs"><button  class='btn' onclick="demonstrateOperate();"><span><span>删除所有</span></span></button></security:authorize>
		</s:if><s:else>
