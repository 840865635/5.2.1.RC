...
function updateView(id){
	if(versionType=="online"){
		if(parent.processName.indexOf("开发案例")>0||(parent.processName.indexOf("权限系统")<0&&parent.processName.indexOf("工作流")<0
				&&parent.processName.indexOf("门户")<0&&parent.processName.indexOf("待办事宜")<0&&parent.processName.indexOf("平台系统")<0
				&&parent.processName.indexOf("系统元数据管理")<0&&parent.processName.indexOf("基础设置系统")<0)){
			...
			alert("为确保系统的正常演示，只能修改开发案例系统和自定义系统的数据");
		}
	}else{
		if(id!=""&&typeof(id)!='undefined'){
			$("#view_id").attr("value",id);
